...
class DQNEmpowermentFromConfig(agent.Agent):
  """DQN agent.
  ...
  def __init__(
      ...
      environment_spec: specs.EnvironmentSpec,
      network: networks_lib.FeedForwardNetwork,
      config: dqn_config.DQNEmpowermentConfig,
  ):
    """Initialize the agent."""
    # Data is communicated via reverb replay.

    reverb_replay = replay.make_reverb_prioritized_sequence_replay(
        environment_spec=environment_spec, #no extra spec since we don't want to store LSTM state
        ...
        sequence_period=config.sequence_period,
    )
    self._server = reverb_replay.server
