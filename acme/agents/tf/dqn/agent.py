...
class DQNEmpowerment(agent.Agent):
  ...
  def __init__(
    ...
    # Ensure that we create the variables before proceeding (maybe not needed).
    tf2_utils.create_variables(Qnetwork, [environment_spec.observations])
    tf2_utils.create_variables(target_Qnetwork, [environment_spec.observations])
    double_environment_spec = tf.TensorSpec(shape=[feat_dims * 2], dtype=environment_spec.observations.dtype)
    single_environment_spec = tf.TensorSpec(shape=[feat_dims], dtype=environment_spec.observations.dtype)
    ...
    tf2_utils.create_variables(feat_network, [environment_spec.observations])#,time_series=True)
