...
def add_timestep_dim(nest: types.NestedTensor) -> types.NestedTensor:
  return tree.map_structure(lambda x: tf.tile(tf.expand_dims(x,1),[1,2] + [1 for _ in range(x.shape.rank-1)]),nest)
...
def create_variables(
  ...
  if time_series:
    dummy_output = network(*add_timestep_dim(add_batch_dim(dummy_input))) #add an extra dim for timesteps
  else:
